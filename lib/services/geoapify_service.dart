...
class GeoapifyService {
  ...
  static const String _routingUrl = 'https://api.geoapify.com/v1/routing';
  ...
  /// Calcula uma rota real (seguindo estradas) entre vários waypoints.
  ///
  /// [waypoints] — lista ordenada de pontos a visitar; mínimo 2.
  /// [mode] — modo de transporte: `drive`, `walk`, `bicycle`, `truck`, etc.
  ///
  /// Devolve um [RouteResult] com a polyline para desenhar no flutter_map,
  /// distância total e tempo estimado.
  Future<RouteResult> calcularRota({
    ...
    if (waypoints.length < 2) {
      throw Exception('São necessários pelo menos 2 pontos para calcular uma rota.');
    }

    // Geoapify aceita waypoints no formato "lat,lng|lat,lng|..."
    final waypointsStr =
    ...
    try {
      ...
      if (features == null || features.isEmpty) {
        throw Exception('A Geoapify não devolveu nenhuma rota para estes pontos.');
      }
      ...
      // Geometria pode vir como LineString ou MultiLineString
      ...
      if (type == 'LineString') {
        for (final point in coords) {
          // Geoapify devolve [lng, lat]
          polyline.add(LatLng((point[1] as num).toDouble(), (point[0] as num).toDouble()));
        }
      } else if (type == 'MultiLineString') {
        for (final segment in coords) {
          for (final point in segment as List) {
            polyline.add(LatLng((point[1] as num).toDouble(), (point[0] as num).toDouble()));
          }
        }
      }
      ...
      return RouteResult(
        polyline: polyline,
        distanciaMetros: (properties['distance'] as num).toDouble(),
        tempoSegundos: (properties['time'] as num).toDouble(),
      );
    } on DioException catch (e) {
      throw Exception(_formatarErroDio(e));
    } catch (e) {
